...
/* ----------------- Tasty Recipes ----------------- */
function parseTastyIngredients($) {
  // Pinch of Yum / WP Tasty
  const list = $(
...
export async function GET(req) {
  ...
  try {
    ...
    // 2) Plugin parsers (Tasty → WPRM)
    let ingredients = parseTastyIngredients($);
    let steps = parseTastySteps($);

    if (!ingredients.length) ingredients = parseWprmIngredients($);
    if (!steps.length) steps = parseWprmSteps($);
    ...
    if ((!ingredients.length || !steps.length) && !/\/amp\/?$/.test(target)) {
      try {
        ...
        if (!ingredients.length) {
          ingredients = parseTastyIngredients($amp);
          if (!ingredients.length) ingredients = parseWprmIngredients($amp);
          if (!ingredients.length) ingredients = parseGenericIngredients($amp);
        }
        if (!steps.length) {
          steps = parseTastySteps($amp);
          if (!steps.length) steps = parseWprmSteps($amp);
